...
sub introduce {
  my $self = shift;
  my $sess = $self->session('user_id');
  my @params = $self->param(['title3', 'subtitle3', 'imglist1', 'imglist-desc1','imglist2', 'imglist-desc2','imglist3', 'imglist-desc3','imglist4', 'imglist-desc4','imglist5', 'imglist-desc5','imglist6', 'imglist-desc6','imglist7', 'imglist-desc7','imglist8', 'imglist-desc8',]);
  my $sql = qq{update IndexIntroduce set a = "$params[0]", b = "$params[1]", c = "$params[2]", d = "$params[3]", e = "$params[4]", f = "$params[5]", g = "$params[6]", h = "$params[7]", i = "$params[8]", j = "$params[9]", k = "$params[10]", l = "$params[11]", m = "$params[12]", n = "$params[13]", o = "$params[14]", p = "$params[15]", q = "$params[16]", r = "$params[17]";};
  if ($sess) {
    my $dbh = DBI->connect("DBI:mysql:database=$CMSConfig::database;host=$CMSConfig::host","$CMSConfig::user","$CMSConfig::pass", {RaiseError => 1, AutoCommit => 0});
    $dbh->do($sql);
    $dbh->commit;
